...
- iOS アプリだけの働き（`src/native.ts`、`ios/App/App/Info.plist`）
  - .jww を「Jw_cad 図面」という書類の種類として登録している。「ファイル」アプリの .jww や、メール・チャットなどの
    共有メニューから「ミテハカール」を選んで開ける（端末の上での名前。App Store での名前は「JWWミテハカール」）
...
4. **アプリを登録する**：App Store Connect → アプリ → ＋ → 新規 App。
   ...
   - 作ったあと「App 情報」に出る **Apple ID（数字）** を、`codemagic.yaml` の `vars` の `APP_STORE_APPLE_ID` に入れて push する
     （このアプリは `6815729877` で設定済み。ビルド番号を TestFlight の続きにする。空の値は書けない）
...
3. 確かめること：「ファイル」アプリの .jww をタップ →共有 →「ミテハカール」で開ける／メールの添付 .jww から開ける／
